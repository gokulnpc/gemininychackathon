...
export function Step5_Caption() {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-[#E8E0DC] p-6"
    >
            ...
            <div
              className={cn(
                ...
                state.selectedCaption === style.id
                  ? 'bg-gray-800 ring-2 ring-[#B08D9F] ring-offset-2'
                  : 'bg-gray-700 hover:bg-gray-600'
              )}
            >
              <CaptionPreview styleId={style.id} preview={style.preview} />
            </div>
            <span className="text-xs font-medium text-[#6B6B6B]">{style.name}</span>
          </motion.button>
        ))}
      </div>
    </motion.div>
