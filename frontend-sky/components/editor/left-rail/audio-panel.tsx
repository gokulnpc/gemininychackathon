...
const PUBLIC_MUSIC_TRACKS: PublicMusicTrack[] = musicTracks
  .filter((track): track is typeof track & { audioFile: string } => typeof track.audioFile === "string" && Boolean(track.audioFile))
  .map((track) => ({
    id: track.id,
    title: track.title,
...
export function AudioPanel({ onInsertAudio }: AudioPanelProps) {
  ...
  const fetchMusicAssets = useCallback(async () => {
    try {
      ...
      const response = (await apiClient.get("/api/v1/assets", { params: { category: "music" } })).data as
        | Asset[]
        | { assets: Asset[] };
      ...
      const nextUrls = Object.fromEntries(
        await Promise.all(
          assets.map(async (asset) => {
            try {
              const urlResponse = (await apiClient.get(`/api/v1/assets/${asset.id}/url`, {
  ...
  const handlePlay = useCallback(
    async (trackId: string, src: string) => {
      ...
      try {
        ...
        audio.onended = () => {
          audioRef.current = null;
          setPlayingId((currentId) => (currentId === trackId ? null : currentId));
        };
        await audio.play();
  ...
  const filteredMusicAssets = useMemo(
    () => musicAssets.filter((asset) => asset.filename.toLowerCase().includes(search.toLowerCase())),
    [musicAssets, search]
  );

  const audioTabs: { key: AudioTab; label: string; icon: typeof Music4 }[] = [
    { key: "public", label: "Public", icon: Music4 },
    { key: "my-assets", label: "My assets", icon: Music4 },
          ...
          {filteredPublicTracks.map((track) => {
            ...
            return (
              ...
              >
                <button
                  type="button"
                  onClick={() => void handlePlay(`public:${track.id}`, track.src)}
                  className={cn(
                ...
                >
                  {isPlaying ? <div className="h-3 w-3 rounded-sm bg-primary-foreground" /> : <Play className="ml-0.5 h-4 w-4" />}
                </button>

                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-foreground">{track.title}</p>
                  <p className="line-clamp-2 text-xs text-editor-text-muted">{track.description}</p>
                </div>
          ...
          >
            {uploading ? (
              ...
              <>
                ...
                <p className="mt-1 text-xs text-editor-text-muted">MP3, WAV, and other audio files supported</p>
              </>
            )}
          </div>
          <input ref={fileInputRef} type="file" accept="audio/*" className="hidden" onChange={(event) => void handleUpload(event)} />
        </div>
      ) : null}
    </div>
  );
