...
export function Processing() {
  ...
  // Navigate to step 10 (Review & Publish) when animation is done AND pipeline result is ready
  useEffect(() => {
    if (progress < 100) return;
    if (!state.pipelineProjectId) return; // hold until pipeline done
  ...
  }, [progress, state.pipelineProjectId, dispatch]);
  ...
  const waitingForPipeline = progress >= 100 && !state.pipelineProjectId;
